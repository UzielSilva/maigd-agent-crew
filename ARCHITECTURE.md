...
graph TD
    ...
    subgraph Phase1["Phase 1: Story Review"]
        ...
        SV["StoryValidator<br/><i>Reviews for consistency<br/>issues</i>"]
    end
    ...
    SW -->|"2. Full story + commission steps"| SV
    SV -->|"3. AUTO_FIX issues? Loop back"| SW
    SV -->|"4. Clean or flagged story"| User
    User -->|"5. Approve story"| CP

    CP -->|"6. Describe visual"| PD
    PD -->|"7. Base patterns + axes<br/>(or axis gap)"| CP
    CP -->|"8. Axis gap?"| User
    User -->|"9. Decide (revise/assume/simplify/skip)"| CP

    CP -->|"10. Commission list"| EB
    EB -->|"11. Validated payouts"| CP
    CP -->|"12. Commission table"| User
    User -->|"13. Approve commissions"| PD2

    PD2 -->|"14. Pattern JSONs + briefs"| User
    User -->|"15. Approve patterns"| FS
```
...
flowchart TD
    ...
    P1 --> P1Validate["StoryValidator reviews<br/>for consistency"]
    P1Validate -->|"AUTO_FIX issues"| P1Fix["StoryWriter rewrites"]
    P1Fix --> P1Validate
    P1Validate -->|"CLEAN or FLAG_FOR_REVIEW"| P1Review{"User reviews story"}
    ...
    style P1Validate fill:#3498db,color:#fff
    style P1Fix fill:#3498db,color:#fff
...
| **StoryValidator** | Reviews StoryWriter output for consistency issues (dialogue contradictions, timeline errors, character voice shifts, narrative gaps, request/story misalignment, copyright). Auto-fixes obvious issues; flags complex ones for user review. | Phase 1 |
...
| From | To | What |
|------|----|------|
| StoryWriter | StoryValidator | Full story output for consistency review |
| StoryValidator | StoryWriter | Fix instructions for AUTO_FIX issues (max 3 iterations) |
| StoryValidator | User | FLAG_FOR_REVIEW issues presented alongside the story |
